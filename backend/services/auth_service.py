...
def create_refresh_token(user:Users, db:Session, device:str):
    ...
    db.query(RefreshTokens).filter(RefreshTokens.user_id==user.id,RefreshTokens.device==device).delete()#One token per device
    new_refresh_token = RefreshTokens(token=hashed_token,device=device,user_id=user.id)
...
def verify_refresh_token(refresh_token:str,user_id:str,db:Session,device:str):
    stored = db.query(RefreshTokens).filter(RefreshTokens.user_id==user_id,RefreshTokens.device==device).first()
...
def refresh_expired_token(refresh_token:str,db:Session,response:Response,device:str):
    if not refresh_token: raise HTTPException(401,"Unauthorized")
    try:
...
def logout_user(refresh_token:str,db:Session,response:Response,device:str):
    ...
    try:
        payload = jwt.decode(refresh_token,JWT_SECRET_KEY,algorithms=[ALGORITHM])
        db.query(RefreshTokens).filter(RefreshTokens.user_id==payload.get("id"),RefreshTokens.device==device).delete()
        db.commit()
    except JWTError:
        pass
    return {"message":"Logged Out Successfully"}
